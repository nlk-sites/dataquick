...
// Custom action to display form html for Order Apparel Page
add_filter("gform_pre_render_10", "populate_html");
function populate_html($form)
{
    ...
    $apparelId = array(
    	...
    	'Nike Golf Sport Shirt' => array(
    		'Mens' => '244612',
    		'Womens' => '244613'
    		),
    	'Silk Touch Sport Shirt' => array(
    		'Mens' => 'K500',
    		'Womens' => 'L500'
    		),
    	'Mini Zip Hoodie' => array(
    		'unisex' => 'WB - 1000'
    		),
    	'Performance P-Tech Hat' => array(
    		'unisex' => '1-1760'
    		),
    	);
    ...
    if ($current_page == 5)
    {
        foreach($form["fields"] as &$field)
        {
            // apparel item
            if ($field["id"] == 15)
                $ai1[0] = explode("|", rgpost('input_' . $field['id']));
            if ($field["id"] == 57)
                $ai2[0] = explode("|", rgpost('input_' . $field['id']));
            //style
            if ($field["id"] == 52)
                $ai1[1] = explode("|", rgpost('input_' . $field['id']));
            if ($field["id"] == 58)
                $ai2[1] = explode("|", rgpost('input_' . $field['id']));
            //size 1
            if ($field["id"] == 51 && $ai1[0][0] == 'MicroTwill Jacket')
        		$ai1[2] = explode("|", rgpost('input_' . $field['id']));
			if ($field["id"] == 67 && $ai1[0][0] == 'Hanes T-shirt')
        		$ai1[2] = explode("|", rgpost('input_' . $field['id']));
			if ($field["id"] == 68 && $ai1[0][0] == 'Nike Golf Sport Shirt')
        		$ai1[2] = explode("|", rgpost('input_' . $field['id']));
			if ($field["id"] == 69 && $ai1[0][0] == 'Silk Touch Sport Shirt')
        		$ai1[2] = explode("|", rgpost('input_' . $field['id']));
			if ($field["id"] == 70 && $ai1[0][0] == 'Mini Zip Hoodie') {
        		$ai1[2] = explode("|", rgpost('input_' . $field['id']));
        		$ai2[1][0] = 'Unisex';
        	}
    		if ($ai1[0][0] == 'Performance P-Tech Hat') {
    			$ai1[2] = null;
    			$ai2[1][0] = 'Unisex';
    		}

    		//size 2
    		if ($field["id"] == 59 && $ai1[0][0] == 'MicroTwill Jacket')
        		$ai1[2] = explode("|", rgpost('input_' . $field['id']));
			if ($field["id"] == 71 && $ai1[0][0] == 'Hanes T-shirt')
        		$ai1[2] = explode("|", rgpost('input_' . $field['id']));
			if ($field["id"] == 72 && $ai1[0][0] == 'Nike Golf Sport Shirt')
        		$ai1[2] = explode("|", rgpost('input_' . $field['id']));
			if ($field["id"] == 73 && $ai1[0][0] == 'Silk Touch Sport Shirt')
        		$ai1[2] = explode("|", rgpost('input_' . $field['id']));
			if ($field["id"] == 74 && $ai1[0][0] == 'Mini Zip Hoodie')
        		$ai1[2] = explode("|", rgpost('input_' . $field['id']));
    		if ($ai1[0][0] == 'Performance P-Tech Hat')
    			$ai1[2] = null;
            //qty
            if ($field["id"] == 22)
                $ai1[4] = rgpost('input_' . $field['id']);
            if ($field["id"] == 64)
                $ai2[4] = rgpost('input_' . $field['id']);
            //colors 1
            if ($field["id"] == 53 && ($ai1[0][0] == 'Nike Golf Sport Shirt' || $ai1[0][0] == 'Silk Touch Sport Shirt')):
                $ai1[3] = explode("|", rgpost('input_' . $field['id']));
            elseif ($field["id"] == 54 && $ai1[0][0] == 'MicroTwill Jacket'):
                $ai1[3] = explode("|", rgpost('input_' . $field['id']));
            elseif ($field["id"] == 55 && $ai1[0][0] == 'Mini Zip Hoodie'):
                $ai1[3] = explode("|", rgpost('input_' . $field['id']));
            elseif ($field["id"] == 56 && $ai1[0][0] == 'Hanes T-shirt'):
                $ai1[3] = explode("|", rgpost('input_' . $field['id']));
            else:
                //do nothing
            endif;
            //colors 2
            if ($field["id"] == 61 && ($ai2[0][0] == 'Nike Golf Sport Shirt' || $ai2[0][0] == 'Silk Touch Sport Shirt')):
                $ai2[3] = explode("|", rgpost('input_' . $field['id']));
            elseif ($field["id"] == 60 && $ai2[0][0] == 'MicroTwill Jacket'):
                $ai2[3] = explode("|", rgpost('input_' . $field['id']));
            elseif ($field["id"] == 62 && $ai2[0][0] == 'Mini Zip Hoodie'):
                $ai2[3] = explode("|", rgpost('input_' . $field['id']));
            elseif ($field["id"] == 63 && $ai2[0][0] == 'Hanes T-shirt'):
                $ai2[3] = explode("|", rgpost('input_' . $field['id']));
            else:
                //do nothing
            endif;
        ...
        // outside of loop
        //if no options associated
        if ($ai1[0][0] == 'Performance P-Tech Hat') {
            $ai1[1] = $ai1[2] = $ai1[3] = null;
        }
        if ($ai2[0][0] == 'Performance P-Tech Hat') {
            $ai2[1] = $ai2[2] = $ai2[3] = null;
        }
        //apparel 1
        $html_content = "<dl><dt>" . $ai1[0][0] . "</dt>";
        $html_content .= ($ai1[1] != null) ? "<dd>Style: {$ai1[1][0]}</dd>" : null;
        $html_content .= ($ai1[2] != null) ? "<dd>Size: {$ai1[2][0]}</dd>" : null;
        $html_content .= ($ai1[3] != null) ? "<dd>Color: {$ai1[3][0]}</dd>" : null;
        $html_content .= ($ai1[4] != null) ? "<dd>Qty: {$ai1[4]}</dd>" : null;
        $html_content .= "</dl>";
        //apparel 2
        $html_content .= "<dl><dt>" . $ai2[0][0] . "</dt>";
        $html_content .= ($ai2[1] != null) ? "<dd>Style: {$ai2[1][0]}</dd>" : null;
        $html_content .= ($ai2[2] != null) ? "<dd>Size: {$ai2[2][0]}</dd>" : null;
        $html_content .= ($ai2[3] != null) ? "<dd>Color: {$ai2[3][0]}</dd>" : null;
        $html_content .= ($ai2[4] != null) ? "<dd>Qty: {$ai2[4]}</dd>" : null;
        $html_content .= "</dl>";
        ...
        foreach($form["fields"] as &$field)
        {
            ...
            if ($field["id"] == 75)
            {
                $field["content"] = $apparelId[$ai1[0][0]][$ai1[1][0]];
            }
            if ($field["id"] == 76)
            {
                $field["content"] = $apparelId[$ai2[0][0]][$ai2[1][0]];
            }
        }
    }
